...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json(await topGamesService.refreshPrices(body));
  } catch (error) {
    if (isZodError(error)) {
      return zodError(error);
    }
    return NextResponse.json({ error: error instanceof Error ? error.message : "TOP 100 price refresh failed." }, { status: 500 });
  }
}
