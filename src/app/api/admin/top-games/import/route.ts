...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json(await topGamesService.importTopGames(body));
  } catch (error) {
    if (isZodError(error)) {
      return zodError(error);
    }
    return NextResponse.json({ error: error instanceof Error ? error.message : "TOP 100 import failed." }, { status: 500 });
  }
}
