...
The practical production scope is `TopTrackedGame`: a curated TOP 100 Steam list that can be imported and refreshed without touching the full Steam catalog. TOP 100 import no longer depends exclusively on the partial `SteamCatalogEntry` table. It uses the fallback order existing `Game` -> `SteamCatalogEntry` -> Steam Store `appdetails` -> curated TOP 100 metadata, so 100 tracked games can exist even while the searchable Steam catalog is still partial. `GET /api/top-games` returns ranking, coverage and score readiness for web and Android. A TOP 100 score is nullable and returns `insufficient-data` until the game has both player data and a trusted Steam/manual price. In production API mode, missing real player data is exposed as no-data (`currentPlayers=null`, `playerSource="no-data"`, `playerFreshness="missing"`) instead of public mock values.
...
`POST /api/admin/top-games/import` reports `createdFromSteamCatalog`, `createdFromSteamStore`, `createdFromCuratedFallback`, `missingMetadata` and diagnostic `missingFromSteamCatalog` counts. `missingFromSteamCatalog` is not a blocking failure for TOP 100 anymore; it means the importer tried the Steam Store and curated fallbacks. `POST /api/admin/top-games/bootstrap` runs import, player refresh and Steam Store price refresh in sequence. Start with `dryRun=true`. The matching cron route is `GET|POST /api/cron/refresh-top-games`, protected by `CRON_SECRET`.
