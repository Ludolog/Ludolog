...
describe("mobile api client", () => {
  ...
  it("uses fetch on web and does not add Content-Type for GET", async () => {
    ...
    const client = createApiClient("https://ludolog.vercel.app/", createFetchTransport(fetcher as unknown as Fetcher));
    ...
    expect(url).toBe("https://ludolog.vercel.app/api/deals/best?limit=3");
  ...
  it("uses CapacitorHttp transport for native requests", async () => {
    const request = vi.fn(async () => ({
      ...
      headers: {},
      status: 200,
      url: "https://ludolog.vercel.app/api/admin/status"
    }));
  ...
  it("wraps network failures in a readable mobile error", async () => {
    ...
    await expect(client.getAdminStatus()).rejects.toMatchObject({
      baseUrl: "https://ludolog.vercel.app",
  ...
  it("calls the stats overview endpoint", async () => {
    ...
    expect(fetcher.mock.calls[0]?.[0]).toBe("https://ludolog.vercel.app/api/stats/overview");
  });
  ...
  it("calls the game prices endpoint", async () => {
    ...
    expect(fetcher.mock.calls[0]?.[0]).toBe("https://ludolog.vercel.app/api/games/dota-2/prices");
  });
  ...
  it("calls the TOP 100 endpoint with a bounded limit", async () => {
    ...
    expect(fetcher.mock.calls[0]?.[0]).toBe("https://ludolog.vercel.app/api/top-games?limit=12");
  });
  ...
  it("posts catalog imports as JSON and reads the richer response", async () => {
    ...
    const [url, options] = fetcher.mock.calls[0] as unknown as [string, RequestInit];
    const headers = options.headers as Headers;
    expect(url).toBe("https://ludolog.vercel.app/api/games/import");
